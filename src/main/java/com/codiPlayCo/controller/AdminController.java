...
public class AdminController {
	...
	public String panelPrincipal(Model model) {
		try {
			...
			long totalEstudiantes = todosUsuarios.stream()
					.filter(usuario -> usuario.getRol() != null && usuario.getRol().getId() == 3).count();
	...
	public String guardarCurso(@RequestParam("curso") String nombreCurso, @RequestParam("dirigido") String dirigido,
			@RequestParam("asignacionDocente") String asignacionDocente,
	...
	public IRolRepository getRolRepository() {
